  ...
  useEffect(() => {
    ...
    const handleActivity = async () => {
      ...
      if (timeSinceLastCheck > 5000) {
        ...
        console.log(
          "   Redux (encrypted):",
          reduxUserId?.substring(0, 20) + "...",
        );
        console.log(
          "   localStorage (encrypted):",
          storageUserId?.substring(0, 20) + "...",
        );
        ...
        if (!storageUserId) {
          try {
            ...
            if (!userInfo || userInfo.authenticated === false) {
              ...
              setTimeout(() => {
                toastAlert(
                  "✅ Session valid during activity check but user data tampered contact owner to get login access again at 7035413333",
                  "error",
                );
                console.log(
                  "🚨 SECURITY ALERT: localStorage data tampered but session still valid - restoring user_id if provided by server. Login again...",
                  "warning",
                );
              }, 6000);
              if (userInfo.restore_user_id) {
                localStorage.setItem(
                  "user_id",
                  String(userInfo.restore_user_id),
                );
        ...
        // ⬇️ Case 2: localStorage doesn't match Redux
          ...
          console.warn(
            `   localStorage: ${storageUserId?.substring(0, 20)}...`,
          );
          ...
          try {
            ...
            if (!userInfo || userInfo.authenticated === false) {
              ...
              toastAlert(
                "Session invalid. Logging out...contact owner to get login access again at 7035413333",
                "error",
              );
              await logoutUser();
              setTimeout(() => {
              ...
              setTimeout(() => {
                toastAlert(
                  "🚨 SECURITY ALERT: localStorage/ redux data tampered but session still valid - restoring user_id if provided by server. Login again...",
                  "warning",
                );
                console.error(
                  "🚨 SECURITY ALERT: localStorage/ redux store tampered but session still valid - restoring user_id if provided by server",
                );
              }, 3000);
      ...
      if (timeSinceLastActivity > 5 * 60 * 1000) {
        ...
        try {
          const response = await fetch(`${API_BASE}/api/extend-session/`, {
            method: "POST",
            credentials: "include",
          });
