        ...
        <div className="p-6 overflow-y-auto space-y-3 flex-1">
          {loading ? (
            ...
            <p className="text-sm text-gray-400 text-center py-8">
              No discount groups assigned yet
            </p>
          ) : (
            assignments.map((assignment, index) => {
              const selectedGroup = discountGroups.find(
                (g) => String(g.disc_id) === String(assignment.discount_group),
              );
              return (
                <div key={index} className="flex items-center gap-3">
                  ...
                  {selectedGroup && (
                    <span className="text-xs text-gray-400 whitespace-nowrap">
                      {selectedGroup.category?.name ?? "All"} ›{" "}
                      {selectedGroup.sub_category ?? "All"}
                    </span>
                  )}
                  <button
...
const Masters_data = () => {
  ...
  useEffect(() => {
    if (activeTab !== "products" || products.length === 0 || language === "en")
      return;
  ...
  useEffect(() => {
    ...
    apiFetch("/api/product/subcategories/", {
      ...
      .then((data) =>
        setManageSubCategories(Array.isArray(data) ? data : (data?.data ?? [])),
      )
      .catch((err) =>
    ...
    apiFetch("/api/product/brands-by-subcategory/", {
      ...
      .then((data) =>
        setmanageBrands(Array.isArray(data) ? data : (data?.data ?? [])),
      )
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-6">
      ...
      <h1 className="text-2xl font-semibold mb-6 text-gray-800">
        {t("master_data_discount_management")}
      </h1>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-gray-200 bg-white rounded-lg overflow-hidden">
                  ...
                  <div className="px-4 py-2 text-sm font-medium text-gray-700 bg-white">
                    {t("page")}{" "}
                    <span className="text-[#F7941D]">{currentPage}</span>{" "}
                    {t("of")} {totalPages}
                  </div>
            ...
            >
              <table className="w-full text-sm text-left border-collapse min-w-[900px]">
                <thead className="bg-gray-200 text-gray-600 tracking-wide">
                  <tr>
                    <th className="px-2 py-1 font-bold border-b border-gray-200">
                      {t("product_id")}
                    </th>
                    <th className="px-2 py-1 font-bold border-b border-gray-200 min-w-[180px]">
                      {t("product_name")}
                    </th>
                    <th className="px-2 py-1 font-bold border-b border-gray-200">
                      {t("category_label")}
                    ...
                    <th className="px-2 py-1 font-bold border-b border-gray-200 text-center">
                      {t("status_label")}
                    </th>
                    <th className="px-2 py-1 font-bold border-b border-gray-200 text-right">
                      {t("actions_label")}
                    </th>
                  </tr>
                </thead>
                    ...
                    products.map((product) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-medium">
                          <span translate="yes">{product.product_name}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-medium ${product.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-500"}`}
                          >
      ...
      */}
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="flex justify-between mb-5 gap-2">
            <div className="relative w-full lg:max-w-md">
              <Search
                size={18}
                className="absolute left-3 top-7/24 -translate-y-1/2 text-gray-400 pointer-events-none"
              />
              <input
                type="text"
              ...
              <div className="relative">
                ...
                >
                  <span className="truncate">
                    {activeGroup ? (
                      <span translate="yes">{activeGroup.name}</span>
                    ) : (
                      t("select_group")
                    )}
                  </span>
          ...
          {customerLoading ? (
            ...
            <div className="space-y-6">
              {groupedData.map(
                ({ group_name, total, page, total_pages, customers }) => (
                  <div
                    key={group_name}
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50/50 text-gray-400 uppercase text-[10px] font-bold tracking-widest border-b border-gray-100">
                          <tr>
                            ...
                            <th className="px-6 py-4">
                              {t("current_discounts_label")}
                            </th>
                            <th className="px-6 py-4 text-right">
                              {t("individual_action_label")}
                            </th>
                          </tr>
                        </thead>
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                  {customer.discount_groups?.length > 0 ? (
                                    customer.discount_groups.map((dg) => (
                                      <span
                                        key={dg}
                                        className="px-2.5 py-1 bg-blue-50 text-[#0c7c9d] rounded-lg text-[10px] font-bold border border-blue-100"
                                      >
                                        {dg}
                                      </span>
                                    ))
                  ...
                  <div className="px-6 py-4 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex flex-wrap justify-between items-center gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium">
                          {consumerSummary.count} Active Users
                        </p>
                      </div>
                    </div>
                    <button
      ...
      <BulkDiscountModal
        ...
        discountGroups={discountGroups}
        onSaved={() => {
          setBulkTarget(null); /* optionally refetch */
        }}
      />
