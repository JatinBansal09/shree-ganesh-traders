...
const AssociationSection = ({
  ...
  const [selectedIds, setSelectedIds] = useState([]); // ids checked in the add picker
  const [bulkAdding, setBulkAdding] = useState(false);

  const getOptId = (opt) => opt.id ?? opt.cat_id ?? opt.sub_id ?? opt.brand_id;
  ...
  const allFilteredSelected =
    ...
    filteredAvailable.every((opt) => selectedIds.includes(getOptId(opt)));

  const toggleSelectAll = () => {
    if (allFilteredSelected) {
      // Deselect only the currently filtered ones
      const filteredIds = filteredAvailable.map(getOptId);
  ...
  const handleRemove = async (item) => {
    ...
    const targetId =
      item.linked_cat_id ?? item.linked_sub_id ?? item.linked_brand_id;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      {/* Section header */}
      <div
        className={`px-6 py-4 flex items-center justify-between border-b border-gray-100 ${color.bg}`}
      >
      ...
      {showAdd && (
        <div className="border-b border-gray-100 p-4 bg-gray-50/60">
          ...
          {filteredAvailable.length > 0 && (
            <label className="flex items-center gap-2 px-3 py-1.5 mb-1 cursor-pointer select-none">
              <input
                type="checkbox"
              ...
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">
                Select all {search ? "results" : title.toLowerCase()}
              </span>
            </label>
          )}
            ...
            {filteredAvailable.length > 0 ? (
              filteredAvailable.map((opt) => {
                ...
                return (
                  <div
                    key={optId}
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-orange-50 transition-all group"
                  >
                    <label className="flex items-center gap-2.5 flex-1 cursor-pointer select-none min-w-0">
                      <input
                        ...
                        className="w-3.5 h-3.5 shrink-0 rounded border-gray-300 text-orange-500 focus:ring-orange-400 cursor-pointer"
                      />
                      <span className="text-xs font-medium text-gray-700 group-hover:text-orange-700 truncate">
                        {getLabel(opt)}
                      </span>
                    </label>
                    <button
                      onClick={() => handleAdd(opt)}
                      className="shrink-0 opacity-0 group-hover:opacity-100 text-orange-400 hover:text-orange-600 text-xs font-medium transition-opacity ml-2"
                    >
                      Link →
                    </button>
                  </div>
              ...
              <p className="text-center text-xs text-gray-400 py-4 italic">
                {available.length === 0
                  ? `All ${title.toLowerCase()} already linked`
                  : "No results found"}
              </p>
            )}
          </div>
          ...
          {selectedIds.length > 0 && (
            <button
              ...
              disabled={bulkAdding}
              className={`w-full mt-3 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-white shadow-sm transition-all disabled:opacity-60 ${color.btn}`}
            >
              {bulkAdding ? (
...
const DiscountGroupSettingsPage = () => {
  ...
  const handleAdd = async (type, targetId) => {
    try {
      ...
      const res = await apiFetch(
        `/api/discount-groups/${selectedGroup.disc_id}/associations/`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ type, target_id: targetIds }),
        },
      );
      if (res?.success === false) {
        toastAlert(res.error || "Failed to link", "error");
        return;
      }
  ...
  const handleRemove = async (type, targetId) => {
    try {
      ...
      if (res?.success === false) {
        toastAlert(res.error || "Failed to remove", "error");
        return;
      }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4 sticky top-0 z-10 shadow-sm">
        ...
        <div>
          <h1 className="text-base font-bold text-gray-800">
            Discount Group Associations
          </h1>
          <p className="text-[11px] text-gray-400">
            Manage which categories, subcategories, and brands each discount
            group applies to
          </p>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {groupsLoading ? (
              [...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="mx-3 mb-2 h-14 bg-gray-100 rounded-xl animate-pulse"
                />
              ))
          ...
          {!selectedGroup ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-gray-400 max-w-xs">
                Choose a group from the left panel to view and manage its
                category, subcategory, and brand associations.
              </p>
            </div>
          ) : assocLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3">
              ...
              <p className="text-sm text-gray-500">
                Failed to load associations. Try again.
              </p>
              <button
